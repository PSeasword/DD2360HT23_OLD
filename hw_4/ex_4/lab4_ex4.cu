...
int main(int argc, char **argv) {
  ...
  if (concurrentAccessQ) {
    ...
    gpuCheck(cudaMemPrefetchAsync(A, nzv * sizeof(double), 0));
    gpuCheck(cudaMemPrefetchAsync(ARowPtr, (dimX + 1) * sizeof(int), 0));
    gpuCheck(cudaMemPrefetchAsync(AColIndx, nzv * sizeof(int), 0));
  ...
  cusparseCheck(cusparseCreateCsr(&ADescriptor, dimX, dimX, nzv, ARowPtr, AColIndx, A, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F)); // Make const?
  cusparseCheck(cusparseCreateDnVec(&tempDescriptor, dimX, temp, CUDA_R_64F)); // Make const?
  cusparseCheck(cusparseCreateDnVec(&tmpDescriptor, dimX, tmp, CUDA_R_64F));

  //@@ Insert code to call cusparse api to get the buffer size needed by the sparse matrix per vector (SMPV) CSR routine of cuSPARS
  cusparseCheck(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, ADescriptor, tempDescriptor, &zero, tmpDescriptor, CUDA_R_64F, CUSPARSE_SPMV_CSR_ALG1, &bufferSize));
  ...
  for (int it = 0; it < nsteps; ++it) {
    //@@ Insert code to call cusparse api to compute the SMPV (sparse matrix multiplication) for the CSR matrix using cuSPARSE
    //@@ This calculation corresponds to: tmp = 1 * A * temp + 0 * tmp
    cusparseCheck(cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, ADescriptor, tempDescriptor, &zero, tmpDescriptor, CUDA_R_64F, CUSPARSE_SPMV_CSR_ALG1, buffer));

    // Element multiplications in 1 * A * temp (between sparse and dense):    nzv
    // Element additions in 1 * A * temp (between sparse and dense):          nzv - dimX
    // tmp =:                                                                 dimX
    countFLOP += 2 * nzv;
    ...
    // Multiplications (square) in ||temp||:    dimX
    // Additions in ||temp||:                   dimX - 1
    // Square root in ||temp||:                 1
    countFLOP += 2 * dimX;
  ...
  cputimer_stop("Computing the SMPV");
  printf("FLOP count in SMPV: %i\n", countFLOP);
  ...
  one = -1;
  //@@ Insert the code to call cublas api to compute the difference between the exact solution and the approximation
